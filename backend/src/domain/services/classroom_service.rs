...
use crate::domain::{
    ...
    ports::classroom_repository::ClassroomRepository, validation::normalize_required_text,
};
...
impl ClassroomService {
    ...
    pub async fn find_by_name_and_building(
        &self,
        name: &str,
        building_id: i32,
    ...
    pub async fn create(
            ...
            .is_some()
        {
            return Err(DomainError::Conflict(
                "El aula ya existe en ese edificio".to_string(),
            ));
        }
    ...
    pub async fn update(
        ...
        if let Some(existing) = self
            .repo
            .find_by_name_and_building(&current.name, building_id)
            .await?
        {
            if existing.id != id {
                return Err(DomainError::Conflict(
        ...
        self.repo
            .update(id, Some(&current.name), Some(current.building_id))
            .await
    }
